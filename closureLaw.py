...
def DCdUd_swananda(alpha, rhol, rhog, ul, ug, theta, DH, AREA, EPS, G, MUL, MUG, sigma, w_u, w_rho, tol):
	"""
	Derivada do parâmetro de distribuição C0 e da velocidade de deslizamento Ud
	em relação a alpha, rho_l, rho_g, u_l e u_g
	...
	if (Fr <= 0.1) and (theta < 0) and (theta > -50 * np.pi / 180):
	   ...
	   t42 = t36 ** 0.5
	   t43 = dxda
	   dC01da = t37 * (0.15 * (-t10 * ug + (ug - ul) / t15 * t6) * t13 - t30 * dftpdRetp / t27 / 0.2) * t5 - 1.5 * t43 * t42 * (t39 - t27) * t5
	   
	   t1 = 1 / rhol
	   t3 = np.sqrt(t1 * rhog)
	   t6 = alpha * ug
	   t13 = (2.6 - (1 / (t6 + (1 - alpha) * ul) * t6)) ** 0.15
	   t14 = Retp
	   t17 = ftp
	   t18 = np.sqrt(t17)
	   t19 = t13 - t18
	   ...
	   t30 = t22 ** 0.5
	   t31 = dxdrhog
	   dC01drhog = -t23 * t19 * t1 / t3 * C1 / 0.2 - 1.5 * t31 * t30 * t19 * (-t3 * C1 + C1)
	   
	   t3 = np.sqrt(rhog / rhol)
	   t7 = rhol ** 2
	   t9 = alpha * ug
	   t16 = (2.6 - (1 / (t9 + (1 - alpha) * ul) * t9)) ** 0.15
	   t17 = Retp
	   t20 = ftp
	   t21 = np.sqrt(t20)
	   t22 = t16 - t21
	   ...
	   t40 = t25 ** 0.5
	   t41 = dxdrhol
	   dC01drhol = t26 * t22 / t7 * rhog / t3 * C1 / 0.2 - t26 * t34 * dftpdRetp / t21 * t31 / 0.2 - 1.5 * t41 * t40 * t22 * t31
	   ...
	   t40 = t34 ** 0.5
	   t41 = dxdul
	   dC01dul = t35 * (0.15 * t7 / t15 * ug * alpha * t13 - t28 * dftpdRetp / t25 / 0.2) * t5 - 1.5 * t41 * t40 * (t37 - t25) * t5
	   ...
	   t42 = t36 ** 0.5
	   t43 = dxdug
	   dC01dug = t37 * (0.15 * (-t10 * alpha + 1 / t17 * ug * t15) * t13 - t30 * dftpdRetp / t27 / 0.2) * t5 - 1.5 * t43 * t42 * (t39 - t27) * t5
	...
	t38 = C01
	t40 = t35 ** 2
	dC0da = -t13 * t6 / t10 * (2 - t4) / 5e5 + 1 / t35 * (-t28 * t27 / 5 + t31) + 2e7 * t13 / t7 / t6 / t40 * (t27 + t38)
	...
	t13 = np.cos(theta)
	t15 = t13 * t2 * t11 + 1
	t19 = np.sqrt(1 / (1 + t13) * t15)
	t20 = 1 - alpha
	t21 = t19 ** t20
	t22 = t21 ** 0.2
	t23 = t22 ** 2
	t30 = dC01drhog
	dC0drhog = -2 / (1 + t5 / 1e7) * t3 + 1 / (1 + 1e7 / t5) * (2 / 5 * t13 * t3 / t15 * t20 * t23 + t30)
	...
	t47 = C01
	t49 = t44 ** 2
	dC0drhol = 2 / t9 * t5 - t19 * t6 / t16 * (2 - t14) / 5e5 + 1 / t44 * (-2 / 5 * t23 * t5 / t25 * t30 * t33 + t40) + 2e7 * t19 / t7 / t6 / t49 * (t33 + t47)
	...
	t20 = 1 + 1e7 / t7
	t23 = np.cos(theta)
	t29 = np.sqrt(1 / (1 + t23) * (t23 * t4 + 1))
	t31 = t29 ** (1 - alpha)
	t32 = t31 ** 0.2
	t33 = t32 ** 2
	t34 = C01
	t36 = t20 ** 2
	dC0dul = -t13 * t6 / t10 * (2 - t4) / 5e5 + 1 / t20 * t17 + 2e7 * t13 / t7 / t6 / t36 * (t33 + t34)
	...
	t34 = C01
	t36 = t20 ** 2
	dC0dug = -t13 * t6 / t10 * (2 - t4) / 5e5 + 1 / t20 * t17 + 2e7 * t13 / t7 / t6 / t36 * (t33 + t34)
	...
	t14 = (1 - alpha) ** -0.5
	t16 = C3
	dUdda = -0.5 * C4 * t16 * C2 * t14 * t11 * (0.35 * t1 + 0.45 * t3)
	...
	t18 = C3
	t27 = dC3drhog
	dUddrhog = -t8 * G * DH * C4 * t18 * C2 * t15 / t11 * t5 / 0.2 + C4 * t27 * C2 * t15 * t11 * t5
	...
	t21 = rhol ** 2
	t32 = dC3drhol
	dUddrhol = (t8 * G * DH - 1 / t21 * G * t7) * C4 * t17 * C2 * t15 / t11 * t5 / 0.2 + C4 * t32 * C2 * t15 * t11 * t5
	...
	return dC0da, dC0drhog, dC0drhol, dC0dul, dC0dug, dUdda, dUddrhog, dUddrhol, dUddul, dUddug
...
def DRelEquiLocal_pipe_comp(alpha, rhol, rhog, ul, ug, theta, D, AREA, EPS, G, MUL, MUG, sigma, w_u, w_rho, tol):
    ...
    # Derivadas do fator de atrito para o gás
    dfgdreg, dfgdepsg = voidFraction.Dffan(Reg, EPS / Dg)
    
    # Derivadas do número de Reynolds do líquido em relação a alpha
    t3 = (w_rho * w_u * D * rhol)
    t5 = (alpha - 1) * ul
    t6 = np.sign(t5)
    t8 = 1 / MUL
    ...
    # Derivadas do número de Reynolds do gás em relação a alpha
    t3 = (w_rho * w_u * D * rhog)
    t4 = (alpha * ug)
    t5 = np.sign(t4)
    t7 = 1 / MUG
    ...
    # Derivada de ui em relação a ul
    if Rel < 2000:
        duidul = 1.8
    elif Rel > 2200:
        duidul = 1.0
    ...
    t83 = t82 * t74
    t88 = t51 ** 2
    dfda = (t17 * t16 * ug * (t1 * dfgdreg - t7 / t5 * dfgdepsg * EPS) * rhog) / 0.2 - \
      (t17 * t27 * t13 * t26) / 0.2 - (t33 * t16 * t26) / 0.2 - \
      (t53 * t50 * ul * (t37 * dfldrel - t43 / t41 * dfldepsl * EPS) * rhol) - \
      (t53 * t27 * t49 * t61) - (2 / t65 * t50 * t61) + \
      (t77 * t17 * t75 * t74 * t73) / 0.2 - \
      (t77 * t33 * t83 * t73) / 0.2 + (1 / t88 * t17 * t83 * t73) / 0.2
    ...
    t28 = gammai
    t35 = np.sin(theta)
    dfdrhog = (t11 * t9 * t7 * ug * t5) / 0.2 + (t11 * t9 * t7 * ug * t16 * dfgdreg * rhog) / 0.2 + \
         (1 / (1 - alpha) * t11 * t28 * t26 * t24 * fi) / 0.2 - t35 * PID
    ...
    t37 = 1 / t7 / alpha
    t43 = np.sign(t31)
    dfdul = -t9 * t6 * t4 * ul * t2 * dfldrel * rhol - t9 * t6 * t4 * t18 - t9 * t6 * t23 * ul * t18 - \
       (t37 * t33 * t32 * t28 * t27) / 0.2 - (t37 * t33 * t43 * t28 * t31 * t27) / 0.2
    ...
    t34 = 1 / (1 - alpha)
    t38 = np.sign(t28)
    dfdug = (t8 * t7 * t4 * ug * t2 * dfgdreg * rhog) / 0.2 + (t34 * t8 * t31 * t38 * t28 * t26) / 0.2 + \
       (t8 * t7 * t22 * ug * t17) / 0.2 + (t34 * t8 * t31 * t29 * t26) / 0.2 + (t8 * t7 * t4 * t17) / 0.2
